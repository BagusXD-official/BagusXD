...
*Pindai kode QR ini untuk menjadi Bot (SUB BOT), Anda dapat menggunakan perangkat lain untuk memindai*
...
*Anda dapat mengirimkan ID yang diberikan ke bot secara pribadi untuk menghubungkan kembali bot tanpa harus memindai kode lagi, kode ini dimulai dengan .jadibot.*
...
let handler = async (m, {
    ...
    exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
        ...
        async function jddt() {
            ...
            if (!fs.existsSync("./jadibot/sessi/" + uniqid)) {
                fs.mkdirSync("./jadibot/sessi/" + uniqid, {
                    recursive: true
                });
            }
            args[0] ? fs.writeFileSync("./jadibot/sessi/" + uniqid + "/creds.json", JSON.stringify(JSON.parse(Buffer.from(args[0], "base64").toString("utf-8")), null, '\t')) : ""
            const {
                state,
                saveState,
                saveCreds
            } = await useMultiFileAuthState("./jadibot/sessi/" + uniqid)
            ...
            async function connectionUpdate(update) {
                ...
                if (connection == 'open') {
                    ...
                    if (!args[0]) parentw.sendMessage(m.chat, {
                        text: usedPrefix + command + " " + Buffer.from(fs.readFileSync("./jadibot/sessi/" + uniqid + "/creds.json"), "utf-8").toString("base64")
                    }, {
                        quoted: m
                    })
                }
