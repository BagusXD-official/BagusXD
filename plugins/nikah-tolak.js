...
let handler = async (m, { conn, text, participants, groupMetadata }) => {
  ...
  try {
    ...
    if(global.db.data.users[user].pasangan != m.sender){
      conn.reply(m.chat,`Maaf @${user.split('@')[0]} Tidak Sedang Mengajak Nikah Kamu`,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }else{
      global.db.data.users[user].pasangan = ""
      conn.reply(m.chat,`Kamu Baru Saja Menolak @${user.split('@')[0]} Untuk Menjadi Kekasih Kamu!`,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }
	}	
