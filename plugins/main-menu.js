...
══════════════════
Pengigat : *Jangan Lupa Makan*
══════════════════
*BOT SEDANG DALAM PENGEMBANGAN*
PROGRES : *65%*
Fitur Yang Akan Rilis :
•Bug Menu : Released 8 Maret
•Jkt48 Random : Released 10 Maret
•Kenon,Unbanned : Released 13 Maret
•Other features : Random Released
 ══════════════════
...
let ramadhan = () => {
  ...
  return hariKe > 0 ? `Selamat Menunaikan Ramadhan Hari ke ${hariKe}` : `Hari ke ${Math.abs(hariKe)} menuju Ramadhan`;
    ...
    let replace = {
      ...
      platform, wib, mode, _p, money, age, ramadhan, tag, name, prems, level, limit, name, weton, week, date, dateIslamic, time, totalreg, rtotalreg, role,
      readmore: readMore
    }
    text = text.replace(new RegExp(`%(${Object.keys(replace).sort((a, b) => b.length - a.length).join`|`})`, 'g'), (_, name) => '' + replace[name])
    
 let fkon = { key: { fromMe: false, participant: `${m.sender.split`@`[0]}@s.whatsapp.net`, ...(m.chat ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { contactMessage: { displayName: `${name}`, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;a,;;;\nFN:${name}\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`}}}
 let fotonya = 'https://telegra.ph/file/7517dfdca23570f039309.jpg'
 await conn.sendFile(m.chat, fotonya, 'conco.jpg', text, m)
