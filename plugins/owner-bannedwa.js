...
let handler = async (m, { conn, text }) => {
	if (!text) {
		return m.reply("Nomornya Mana yang akan di nonaktifkan?");
	}
	// replace no numeric
...
Kami telah menonaktifkan akun WhatsApp Anda.`.trim()
			...
			m.reply(
				`Terima kasih telah melaporkan akun yang mencurigakan.
kami akan segera menindak lanjuti akun tersebut.
Kami akan menghubungi Anda kembali melalui email setelah menonaktifkan akun yang di laporkan, dan itu mungkin memerlukan waktu hingga dua hari kerja..`.trim()
			);
