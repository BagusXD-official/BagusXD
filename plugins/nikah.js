...
let ps = groupMetadata.participants.map(v => v.id)
  ...
  try {
    ...
    if(user === m.sender) return conn.reply(m.chat, `_*Tidak Bisa Menikah Dengan Diri Sendiri.*_`, m)
    if(user === conn.user.jid) return conn.reply(m.chat, `_*Maaf Kak, Tapi Saya Sepenuhnya Milik Andini. :')*_`, m)
    ...
    if(global.db.data.users[m.sender].pasangan != "" && global.db.data.users[global.db.data.users[m.sender].pasangan].pasangan == m.sender && global.db.data.users[m.sender].pasangan != user){
      conn.reply(m.chat, `Kamu Sudah Menikah Dengan @${global.db.data.users[m.sender].pasangan.split('@')[0]}\n\nSilahkan Cerai Dulu (ketik .cerai Untuk Memutuskan Hubungan) Untuk Menikah Dengan @${user.split('@')[0]}\n\nBtw Yang Setia\nDikit Banget Ya!`, m , { contextInfo: { mentionedJid: [user, global.db.data.users[m.sender].pasangan]}})
    }else if(global.db.data.users[user].pasangan != ""){
      if (pacar){
        if (m.sender == pacar && global.db.data.users[m.sender].pasangan == user) return conn.reply(m.chat, `Kamu Sudah Menikah Dengan ${spac.split('@')[0]}`, m , { contextInfo: { mentionedJid: [spac]}})
        conn.reply(m.chat, `Maaf, @${user.split('@')[0]} Sudah Menikah Dengan @${pacar.split('@')[0]}\nSilahkan Cari Pasangan Lain!`, m , { contextInfo: { mentionedJid: [user, pacar]}})
      }else{
        global.db.data.users[m.sender].pasangan = user
        conn.reply(m.chat, `${await ktnmbk.getRandom()}\n\nKamu Baru Saja Mengajak @${user.split('@')[0]} Menikah\n\nSilahkan Menunggu Jawaban Darinya!\n\nKetik *${usedPrefix}terimanikah @user* Untuk Menerima\n*${usedPrefix}tolaknikah @user Untuk Menolak*`, m , { contextInfo: { mentionedJid: [user]}})
      }
    }else if (global.db.data.users[user].pasangan == m.sender){
      global.db.data.users[m.sender].pasangan = user
      conn.reply(m.chat, `Selamat Atas Pernikahan Kalian, Selamat Kamu Resmi Menikah Dengan @${user.split('@')[0]}\n\nsemoga Menjadi Keluarga Yang Sakinah, Mawaddah, Warahmah. 🥰🥰`, m , { contextInfo: { mentionedJid: [user]}})
    }else {
      global.db.data.users[m.sender].pasangan = user
      conn.reply(m.chat, `${await ktnmbk.getRandom()}\n\nKamu Baru Saja Mengajak @${user.split('@')[0]} Menikah\n\nSilahkan Menunggu Jawaban Darinya!\n\nKetik *${usedPrefix}terimanikah @user* Untuk Menerima\n*${usedPrefix}tolaknikah @user Untuk Menolak*`, m , { contextInfo: { mentionedJid: [user]}})
    }
...
handler.help = ['menikah']
handler.tags = ['group']
handler.command = /^(nikah|menikah)$/i
handler.group = true
...
let ktnmbk = [
"Ada saat di mana aku nggak suka sendiri. Tapi aku juga nggak mau semua orang menemani, hanya kamu yang kumau. mau kah kamu menjadi pasangan hidupku?",
"Aku baru sadar ternyata selama ini kamu kaya! Kaya yang aku cari selama ini. Kamu mau nggak jadi pasanganku?",
"Aku berterima kasih pada mataku, sebab mata inilah yang menuntunku untuk menemukanmu.",
"Aku boleh kirim CV ke kamu nggak? Soalnya aku mau ngelamar jadi pasanganmu.",
...
