...
Kamu *${status}*, Kamu ${status == 'Menang' ? `Mendapatkan Uang Sebesar *Rp.${count * 2}*` : status == 'Kalah' ? `Kehilangan Uang Sebesar *$.${count * 1}*` : `Mendapatkan Uang *$.${Math.floor(count / 1.5)}*`} 
  `.trim())
            clearTimeout(timeout)
            delete confirm[m.sender]
            return !0
